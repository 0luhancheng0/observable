...
function dragRectObservable() {
  ...
  dragRectObservable_aux(<HTMLElement>document.getElementById('dragRect'))
}

function dragRectObservable_aux(svg: HTMLElement) {
  const
    mousemove = Observable.fromEvent<MouseEvent>(svg, 'mousemove'),
    mouseup = Observable.fromEvent<MouseEvent>(svg, 'mouseup'),
    rect = new Elem(svg, 'rect')
  ...
  rect.observe<MouseEvent>('mousedown')
    ...
    .subscribe(({ x, y }) =>
      rect.attr('x', x)
        .attr('y', y));
  // const mousemove_observable = Observable.fromEvent<MouseEvent>(svg, 'mousemove')
  // const mousedown_observable = rect.observe<MouseEvent>('mousedown')
  // drag_any_rect(svg, rect, mousemove_observable, mousedown_observable)
}

// function drag_any_rect(svg:HTMLElement, rect:Elem, mousemove_observable: Observable<MouseEvent>, mousedown_observable: Observable<MouseEvent>) {
//   const mouseup_observable = Observable.fromEvent<MouseEvent>(svg, 'mouseup')
//   mousedown_observable
//     .map(({ clientX, clientY }) => ({
//       xOffset: Number(rect.attr('x')) - clientX,
//       yOffset: Number(rect.attr('y')) - clientY
//     }))
//     .flatMap(({ xOffset, yOffset }) =>
//     mousemove_observable
//         .takeUntil(mouseup_observable)
//         .map(({ clientX, clientY }) => ({ x: clientX + xOffset, y: clientY + yOffset })))
//     .subscribe(({ x, y }) =>{
//       rect.attr('x', x)
//         .attr('y', y)
...
function drawRectsObservable() {
  ...
  // const svg = document.getElementById("drawRects")!;
  // const svg_observable = Observable.fromEvent<MouseEvent>(svg, 'mousedown')
  // svg_observable.subscribe(mouse_down_event => {
  //   const svgRect = svg.getBoundingClientRect(),
  //     rect = new Elem(svg, 'rect'),
  //     draw_rect = Observable.fromEvent<MouseEvent>(svg, 'mousemove'),
  //     stop_drawing_observable = Observable.fromEvent<MouseEvent>(svg, 'mouseup'),
  //     x0 = mouse_down_event.clientX - svgRect.left,
  //     y0 = mouse_down_event.clientY - svgRect.top
  //   const stop_drawing = draw_rect.subscribe(mouse_move_event => {
  //     const 
  //       x1 = mouse_move_event.clientX - svgRect.left,
  //       y1 = mouse_move_event.clientY - svgRect.top,
  //       x_pos = Math.min(x0, x1),
  //       y_pos = Math.min(y0, y1),
  //       width = Math.abs(x0 - x1),
  //       height = Math.abs(y0 - y1)
  ...
  //       .attr('fill', '#95B3D7')
  //   })
  //   stop_drawing_observable.subscribe(_=> {
  //     stop_drawing()
  //   })

  // })
  drawRectsObservable_aux(<HTMLElement>document.getElementById('drawRects'))
}

function drawRectsObservable_aux(svg: HTMLElement) {

  const svg_observable = Observable.fromEvent<MouseEvent>(svg, 'mousedown')
  svg_observable.subscribe(mouse_down_event => {
    // const svgRect = svg.getBoundingClientRect(),
    const rect = new Elem(svg, 'rect')
    draw(svg, rect, mouse_down_event)
...
function draw(svg: HTMLElement, rect: Elem, mouse_down_event: MouseEvent) {
  const svgRect = svg.getBoundingClientRect()
  const
    draw_rect = Observable.fromEvent<MouseEvent>(svg, 'mousemove'),
    stop_drawing_observable = Observable.fromEvent<MouseEvent>(svg, 'mouseup'),
...
function draw_and_drag(svg: HTMLElement, rect: Elem, mouse_down_event: MouseEvent) {
  const svgRect = svg.getBoundingClientRect()
  const
    draw_rect = Observable.fromEvent<MouseEvent>(svg, 'mousemove'),
    stop_drawing_observable = Observable.fromEvent<MouseEvent>(svg, 'mouseup'),
...
function drawAndDragRectsObservable() {
  ...
  const 
    svg = <HTMLElement>document.getElementById("drawAndDragRects"), 
    mousedown_observable = Observable.fromEvent<MouseEvent>(svg, 'mousedown')
  mousedown_observable
    .subscribe(mouse_down_event => {
      const hover_at_ele = document.elementFromPoint(mouse_down_event.clientX, mouse_down_event.clientY)
      if (hover_at_ele.tagName !== 'rect') {

        // const mouse_down_observable_for_rect = Observable.fromEvent<MouseEvent>(hover_at_ele, 'mousedown')
        // const rect = new Elem(svg, 'rect')
        // rect.elem = hover_at_ele
        // drag_any_rect(svg, rect, Observable.fromEvent(rect.elem, 'mousemove'), mouse_down_observable_for_rect)

        const rect = new Elem(svg, 'rect')
        const mouse_down_observable = <Observable<MouseEvent>>draw_and_drag(svg, rect, mouse_down_event)
        const mouse_move_observable = Observable.fromEvent<MouseEvent>(svg, 'mousemove')
        const mouse_up_observable = Observable.fromEvent<MouseEvent>(svg, 'mouseup')
        mouse_down_observable.map(({ clientX, clientY }) => ({
          xOffset: Number(rect.attr('x')) - clientX,
          yOffset: Number(rect.attr('y')) - clientY
        })).flatMap(({ xOffset, yOffset }) =>
        mouse_move_observable
          .takeUntil(mouse_up_observable)
          .map(({ clientX, clientY }) => ({ x: clientX + xOffset, y: clientY + yOffset })))
      .subscribe(({ x, y }) =>
        rect.attr('x', x)
          .attr('y', y));
